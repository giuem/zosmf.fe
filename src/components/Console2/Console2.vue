<template>
  <div>
    <h2>控制台        <p style="color:red;font-size:10px">{{info}}</p></h2>
    <a-form :form="form" layout="vertical" @submit="handleSubmit">
      <a-form-item>
        <a-textarea placeholder="输入命令，回车提交" v-decorator="['code']" autosize @change="handleChange"/>
        <a-button
        type="primary"
        html-type="submit"
      >
  ...
  methods: {
    handleSubmit(e) {
      ...
      if (!err) {
        ...
        this.$http.put('/api/catalog',values.code).then((response) =>{

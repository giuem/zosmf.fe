        ...
        <div v-for="(questions, index) in steps" :key="index">
          ...
          >
            <a-form-item
              ...
              :colon="false"
            >
              <a-textarea autosize v-decorator="[`step[${index}]`,{initialValue: caogaos[index]}]"></a-textarea>
            </a-form-item>
            <div>
              <span>
                <a-button
                  ...
                  @click="fuckingnow--;changepage()"
                  >上一步</a-button
                >
                <a-button
                  :disabled="fuckingnow === steps.length - 1"
                  @click="fuckingnow++;question=0;changepage()"
                  >下一步</a-button
                >
              </span>
              <span style="float: right">
                <a-button style="margin-right: 10px" type="primary" @click="caogao"
                  >保存</a-button
                >
                <a-button
                  type="primary"
  ...
  mounted(){
    ...
    this.$http({
              ...
              .then(function (response) {
                  console.log(response)
                  for(var i=0;i<response.body.length;i++){
                    this.steps[2].push(response.body[i].question);
                  }
              })
              .catch(function (error) {
                console.log(error);
    ...
    this.$http({
              ...
              .then(function (response) {
                  console.log(response)
                  for(var i=0;i<response.body.length;i++){
                    this.steps[3].push(response.body[i].question);
                  }
              })
              .catch(function (error) {
                console.log(error);
    ...
    this.$http({
              ...
              .then(function (response) {
                  console.log(response)
                  for(var i=0;i<response.body.length;i++){
                    this.steps[4].push(response.body[i].question);
                  }
              })
              .catch(function (error) {
                console.log(error);
